...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="locations" && $field=="active")
	{
		return 1;
	}
				if($table=="reservations" && $field=="rsvStatus")
	{
		return "ΚΛΕΙΣΜΕΝΟ - ΠΡΟΣ ΕΠΙΒΕΒΑΙΩΣΗ";
	}
				if($table=="reservations" && $field=="createdAt")
	{
		return now();
	}
				if($table=="reservations" && $field=="updatedAt")
	{
		return now();
	}
				if($table=="reservations" && $field=="changesCount")
	{
		return 0;
	}
				if($table=="treatments" && $field=="active")
	{
		return 1;
	}
				if($table=="statuses" && $field=="forReception")
	{
		return 0;
	}
				if($table=="reservationsReception" && $field=="rsvStatus")
	{
		return "ΚΛΕΙΣΜΕΝΟ - ΠΡΟΣ ΕΠΙΒΕΒΑΙΩΣΗ";
	}
				if($table=="reservationsReception" && $field=="createdAt")
	{
		return now();
	}
				if($table=="reservationsReception" && $field=="updatedAt")
	{
		return now();
	}
				if($table=="reservationsReception" && $field=="changesCount")
	{
		return 0;
	}
	return "";

...
class RunnerPage
{
	...
	function buildPaginationControl( $currentPage, $maxPages, $limit ) {
		...
		if($counterstart != 1)
		{
			$pagination.= $this->getPaginationLink(1,"Πρώτο");
			$pagination.= $this->getPaginationLink($counterstart - 1,"Προηγούμενο");
		}
		...
		if( $counterend != $maxPages ) {
			$pagination.= $this->getPaginationLink($counterend + 1,"Επόμενο");
			$pagination.= $this->getPaginationLink($maxPages,"Τελευταίο");
		}
	...
	public function assignSimpleSearch() {
		...
		$this->xt->assign('searchbutton_attrs', 'id="searchButtTop'.$this->id.'" title="'."Εύρεση".'"');
...
//		$settings["requestDelay"] = $option["forceExpire"] && $option["lifeTime"] < 2 ? 30 : 60;
	...
	*/
	...
	protected function getFieldFilterStateEntry( $fieldName, $value, $selectedValuesSet ) {
		...
		$displayValue = $displayValue !== "" ? $displayValue : "(" . "Κενό" . ")";

...
class RegisterPage extends RunnerPage
{
	...
	protected function activateNewUser()
	{
		...
		if( !$rs ) {
			echo "Ο κωδικός πρόσβασης δεν είναι σωστός";
			return;
		}
	...
	protected function checkRegisterData( $strUsername, $strPassword, $strEmail )
	{
		...
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Το όνομα χρήστη δεν μπορεί να είναι κενό";
			$ret = false;
		}
		else if( !$this->checkIfUsernameUnique( $strUsername ) )
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Το όνομα χειριστή"." <i>".runner_htmlspecialchars( $strUsername )."</i> "."υπάρχει ήδη. Παρακαλώ επιλέξτε ένα άλλο όνομα.";
			$ret = false;
		}
		...
		if( Security::emailField() && $this->pSet->appearOnPage( Security::emailField() ) )
		{
			...
			if( !strlen($strEmail) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Παρακαλώ εισάγετε έγκυρη ηλεκτρονική διεύθυνση";
				$ret = false;
			}
			else if( !$this->checkIfEmailUnique( $strEmail ) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Το EMail"." <i>". runner_htmlspecialchars( $strEmail )."</i> "."είναι ήδη καταχωρημένο.";
				$ret = false;
			}
		}
	...
	protected function getPwdStrongFailedMessage()
	{
		...
		if($pwdLen)
		{
			$fmt = "Ο κωδικός προσπέλασης θα πρέπει να είναι τουλάχιστον 777 χαρακτήρες";
		...
		if($pwdUnique)
		{
			$fmt = "Ο κωδικός προσπέλασης θα πρέπει να αποτελείτε από μοναδικούς, όχι όμοιους, χαρακτήρες";
		...
		if($pwdDigits)
		{
			$fmt = "Ο κωδικός προσπέλασης θα πρέπει να αποτελείτε από αριθμούς και σύμβολα  χαρακτήρες";
			$fmt = str_replace("%%", "".$pwdDigits, $fmt);
			$msg.= "<br>".$fmt;
		}
		if(GetGlobalData("pwdUpperLower", false))
		{
			$fmt = "Ο κωδικός πρέπει να αποτελείτε από μικρά και κεφαλαία γράμματα-χαρακτήρες";
			$msg.= "<br>".$fmt;
		}

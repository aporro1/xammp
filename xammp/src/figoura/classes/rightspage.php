...
class RightsPage extends ListPage
{
	...
	function fillGroupsArr()
	{
		...
		$this->groups[-1] = array( "label" => "<"."Διαχειριστής".">" );
		$this->groups[-2] = array( "label" => "<"."Εξ'όρισμού".">" );
		$this->groups[-3] = array( "label" => "<"."Επισκέπτης".">" );
	...
	function getRights()
	{
		...
		$sql = "select ". $this->connection->addFieldWrappers( "GroupID" )
			...
			." from ". $this->connection->addTableWrappers( "figuraugrights" )
	...
	function updateTablePermissions( $table, $group, $tableRights )
	{
		...
		$rightWTableName = $this->connection->addTableWrappers( "figuraugrights" );

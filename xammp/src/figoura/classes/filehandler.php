...
class RunnerFileHandler {
	...
	protected function validateFile( $filename, $fileSize, &$errorMessage ) {
        if ( !$filename ) {
            $errorMessage = "Το όνομα αρχείου δεν δόθηκε";
            return false;
        }
		...
		if( $acceptedTypes ) {
			...
			if( array_search( $ext, $acceptedTypes ) === false && array_search( ".".$ext, $acceptedTypes ) === false ) {
				$errorMessage = "Ο τύπος αρχείου δεν είναι αποδεκτός";
				return false;
			}
		}
		...
		if ( $maxFileSize && $fileSize > $maxFileSize * 1024 ) {
            $errorMessage = mysprintf("Το μέγεθος αρχείου είναι μεγαλύτερο από το όριο %s kbytes", array( $maxFileSize ) );
            return false;
        }
		...
		if ( $maxTotalFileSize ) {
			if( $this->getUploadFilesSize() + $fileSize > $maxTotalFileSize * 1024 ) {
				$errorMessage = mysprintf( "Το σύνολο όλων των αρχείων είναι μεγαλύτερο από το όριο %s kbytes", array( $maxTotalFileSize ) );
				return false;
			}
		}
		...
		if ( $maxNumberOfFiles && $this->getUploadFilesCount() >= $maxNumberOfFiles ) {
			$errorMessage = mysprintf( "Μπορείτε να μεταφορτώσετε μέχρι %s αρχεία", array( $maxNumberOfFiles ) );
			return false;
		}
        return true;
	}

...
class LoginPage extends RunnerPage
{
	...
	protected function prepareTwoFactorMessage() {
		...
		if( $destination["method"] == TWOFACTOR_PHONE ) {
			$twofMessage = str_replace( "%phone%", $destination["address"], "Σας έχει σταλεί ένα μήνυμα κειμένου με τον κωδικό σας στο: %phone%" );
		} else if( $destination["method"] == TWOFACTOR_EMAIL ) {
			$twofMessage = str_replace( "%email%", $destination["address"], "Ένα μήνυμα ηλεκτρονικού ταχυδρομείου με τον κωδικό σας έχει σταλεί στο %email%." );
			...
			$twofMessage = str_replace(
				...
				"Εισάγετε τον κωδικό από την εφαρμογή ελέγχου ταυτότητας που αντιστοιχεί στο %username% στο %site%."
			);
			$this->hideItemType("resend_button");
		}
	...
	protected function sendTwoFactorCode()
	{
		...
		if( !$ret["success"] )
		{
			$this->message = "Σφάλμα κατά την αποστολή του μηνύματος"." ".$ret["message"];
